...
def _load():
    ...
    class _CWindow(ctypes.Structure):
        pass
    lib.ne_win_create.argtypes = [ctypes.c_int, ctypes.c_int, ctypes.c_char_p]
    lib.ne_win_create.restype = ctypes.POINTER(_CWindow)
    lib.ne_win_destroy.argtypes = [ctypes.POINTER(_CWindow)]
    lib.ne_win_pump.argtypes = [ctypes.POINTER(_CWindow)]
    lib.ne_win_pump.restype = ctypes.c_int
    lib.ne_win_key_down.argtypes = [ctypes.POINTER(_CWindow), ctypes.c_int]
    lib.ne_win_key_down.restype = ctypes.c_int
    lib.ne_win_key_pressed.argtypes = [ctypes.POINTER(_CWindow), ctypes.c_int]
    lib.ne_win_key_pressed.restype = ctypes.c_int
    f3 = ctypes.c_float
    lib.ne_win_mouse.argtypes = [ctypes.POINTER(_CWindow)] + [ctypes.POINTER(f3)] * 4 \
                                 + [ctypes.POINTER(ctypes.c_int)] * 2
    lib.ne_win_present.argtypes = [ctypes.POINTER(_CWindow), ctypes.POINTER(ctypes.c_ubyte)]

    lib.ne_raster_resize.argtypes = [P(_CEngine), ctypes.c_int, ctypes.c_int]
    pf = ctypes.POINTER(ctypes.c_float)
    lib.ne_raster_render.argtypes = [P(_CEngine), pf, pf, ctypes.c_float]
    lib.ne_raster_pixels.argtypes = [P(_CEngine), ctypes.POINTER(ctypes.c_int),
    ...
    for _n, _a in (("ne_stream_begin", [ctypes.c_void_p, ctypes.c_int]),
                   ("ne_stream_step", [ctypes.c_void_p, ctypes.c_int]),
                   ("ne_stream_active", [ctypes.c_void_p]),
                   ("ne_stream_passes", [ctypes.c_void_p]),
                   ("ne_stream_present", [ctypes.c_void_p, ctypes.c_int]),
                   ("ne_rt_size", [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p]),
                   ("ne_rt_blit", [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_int,
                                   ctypes.c_int, ctypes.c_int])):
        getattr(lib, _n).argtypes = _a
        if _n in ("ne_stream_step", "ne_stream_active", "ne_stream_passes"):
...
class App:
    ...
    # ------------------------------------------------- streaming (games) ----
    def begin_stream(self, passes=8):
        """(Re)queue progressive tile streaming after a scene edit."""
        self._lib.ne_stream_begin(self._ptr, int(passes))

    def stream_frame(self, budget_ms=4, denoise=True):
        """Render tiles until the time budget is spent, tonemap + denoise.
        The UI never blocks: partial state is always presentable.
        Returns (still_rendering, min_passes_done)."""
        import time as _t
        t0 = _t.perf_counter()
        busy = False
        while self._lib.ne_stream_step(self._ptr, int(budget_ms * 1000)):
            busy = True
            if _t.perf_counter() - t0 > budget_ms / 1000.0:
                break
        self._lib.ne_stream_present(self._ptr, 1 if denoise else 0)
        return busy, self._lib.ne_stream_passes(self._ptr)

    def blit(self, width, height, bilinear=True) -> bytes:
        """Upscale the RT buffer to width x height RGBA bytes (window frame)."""
        buf = (ctypes.c_ubyte * (width * height * 4))()
        self._lib.ne_rt_blit(self._ptr, buf, int(width), int(height), 1 if bilinear else 0)
        return bytes(buf)

    def rt_size(self):
        w, h = ctypes.c_int(), ctypes.c_int()
        self._lib.ne_rt_size(self._ptr, ctypes.byref(w), ctypes.byref(h))
    ...
    # ------------------------------------------------------- realtime raster --
    def raster_resize(self, width=None, height=None):
        self._lib.ne_raster_resize(self._ptr, int(width or self.width),
                                   int(height or self.height))

    def raster_render(self, eye, at, fov=62.0):
        """Rasterize the current scene from eye->at (60 fps path)."""
        e, a = _f3(Vec3(eye)), _f3(Vec3(at))
        self._lib.ne_raster_render(self._ptr, e, a, float(fov))

    def raster_pixels(self):
        """(bytes, w, h) of the last raster frame (RGBA8, HUD included)."""
        w, h = ctypes.c_int(), ctypes.c_int()
        p = self._lib.ne_raster_pixels(self._ptr, ctypes.byref(w), ctypes.byref(h))
        if not p or w.value <= 0:
            return b"", 0, 0
        return bytes(bytearray(ctypes.string_at(p, w.value * h.value * 4))), w.value, h.value

    def hud_text(self, x, y, text, scale=2, color=0xFFFFFF):
        self._lib.ne_hud_text(self._ptr, int(x), int(y), int(scale),
                              text.encode(), int(color))
...
# ---------------------------------------------------------------------------
#  Desktop window + real-time game API
# ---------------------------------------------------------------------------
class Window:
    """Native OS window for real-time gameplay (Win32 on Windows, headless
    elsewhere so the same code runs in CI / over the browser preview)."""

    def __init__(self, width=1024, height=576, title="Native Engine Game", lib=None):
        self._lib = lib or _load()
        self.width, self.height = int(width), int(height)
        self._ptr = self._lib.ne_win_create(self.width, self.height, title.encode())
        if not self._ptr:
            raise RuntimeError("Window creation failed (headless env or bad args)")

    def pump(self) -> bool:
        """Process OS messages; False when the user closed the window."""
        return bool(self._lib.ne_win_pump(self._ptr))

    def key_down(self, vk: int) -> bool:
        return bool(self._lib.ne_win_key_down(self._ptr, int(vk)))

    def key_pressed(self, vk: int) -> bool:
        return bool(self._lib.ne_win_key_pressed(self._ptr, int(vk)))

    def mouse(self):
        lib = self._lib
        x, y, dx, dy = ctypes.c_float(), ctypes.c_float(), ctypes.c_float(), ctypes.c_float()
        lmb, rmb = ctypes.c_int(), ctypes.c_int()
        lib.ne_win_mouse(self._ptr, ctypes.byref(x), ctypes.byref(y), ctypes.byref(dx),
                         ctypes.byref(dy), ctypes.byref(lmb), ctypes.byref(rmb))
        return x.value, y.value, dx.value, dy.value, bool(lmb.value), bool(rmb.value)

    def present(self, rgba: bytes):
        buf = (ctypes.c_ubyte * len(rgba)).from_buffer_copy(rgba)
        self._lib.ne_win_present(self._ptr, buf)

    def close(self):
        if getattr(self, "_ptr", None):
            self._lib.ne_win_destroy(self._ptr)
            self._ptr = None


# convenience: VK codes for WASD etc. (letters == ASCII)
