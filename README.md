...
- **Built-in IDE** — press `IDE` (or F2) for a docked code editor with
  syntax highlighting, a live console and **RUN**: `NEW PY` starts a game in
  Python (`native_engine` API), `NEW CPP` a standalone C++ file the IDE
  builds with your `g++`. Ctrl+R runs, STOP kills the process tree, output
  streams into the console pane. Files live in `projects/`.
- **Python runtime = scripting layer** — `native_engine` package: scene,
  materials, camera, and the *game-loop streaming API*
  (`begin_stream / stream_frame / blit`) so scripts render path-traced RTX
  at game framerates; plus `ne.run_editor()` to launch the native app.
...
· `F1` shows the in-app help card.

**Writing a game (IDE → NEW PY):**

```python
app = ne.App(480, 270)                 # trace at low res …
win = ne.Window(1120, 700, "my game")  # … present at full res
app.begin_stream(32)                   # re-queue after each scene edit
while win.pump():
    ball.set_transform(pos=new_pos)    # stale tiles keep the last frame,
    app.begin_stream(32)               # refreshed tiles blend in — no flash
    app.stream_frame(4)                 # render tiles for <=4 ms, never block
    win.present(app.blit(1120, 700))   # denoised RT upscaled to the window
```
