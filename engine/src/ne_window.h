...
public:
    ...
    bool alive = false;
    std::vector<char> chars;      // typed characters (printable + ctrl codes)

    int take_char() {             // pop oldest typed char, -1 when empty
        if (chars.empty()) return -1;
        char c = chars.front();
        chars.erase(chars.begin());
        return (unsigned char)c;
    }
    void inject_char(char c) { chars.push_back(c); }   // headless tests
...
inline LRESULT CALLBACK Window::wndproc(HWND h, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_CHAR:
        if (wp >= 8 && wp < 127) w->chars.push_back((char)wp);
        return 0;
