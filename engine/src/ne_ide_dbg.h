...
int main() {
    ...
    Obj pl; pl.type = ObjType::Plane; pl.mat = mf; pl.d = 0; s.add_object(pl);
    Obj sp; sp.type = ObjType::Sphere; sp.mat = mm; sp.radius = 1.0f;
    int is = s.add_object(sp);
...
struct Ide {
    ...
    void draw_dock(Ctx& u, int x, int y, int w, int h, double now) {
        if (::g_dbg) fprintf(stderr, "[d] dock start\n");
        caret_blink_t = now;
        ...
        B btns[] = {
            ...
            {"SAVE", 4, 0x1F2B3A},
            {run.running ? "RUNNING" : "RUN  (Ctrl+R)", 5, run.running ? 0x3A2430 : 0x143528},
            {"STOP", 6, 0x351414},
        };
        int hit = -1;
        for (auto& b : btns) {
        ...
        if (::g_dbg) fprintf(stderr, "[d] toolbar done\n");
        // ---- file dropdown
        if (listing) {
            auto files = ide_listdir(dir);
        ...
        if (::g_dbg) fprintf(stderr, "[d] dropdown done\n");
        // ---- code | console split
        ...
        if (::g_dbg) fprintf(stderr, "[d] code done\n");
        // scrollbar for code
        if (max_scroll > 0) {
        ...
        if (::g_dbg) fprintf(stderr, "[d] console done\n");
