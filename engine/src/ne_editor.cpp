...
struct Editor {
    ...
    // ---------------------------------------------------------------- UI ----
    void apply_layout() {
        vp_x = 190; vp_y = 26; vp_w = W - 190 - 236; vp_h = H - 26 - 20;
    }
    ...
    void draw(Ctx& u) {
        ...
        if (button(u, "SNAPSHOT", W - 236 - 200, 5, 86, 16)) snapshot("native_engine_render.png");
        if (button(u, "SAVE", W - 236 - 108, 5, 50, 16)) save_scene("scene.nes");
        if (button(u, "LOAD", W - 236 - 54, 5, 50, 16)) load_scene("scene.nes");
        if (status_t > 0) { u.text(300, 10, status.c_str(), 2, GOLD); }
...
struct App : ed::Editor {
    ...
    bool open(int w, int h) {
        W = w; H = h;
        if (!win.create("Native Engine - RTX Editor", w, h)) return false;
