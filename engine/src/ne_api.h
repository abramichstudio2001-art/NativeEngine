...
/* Progressive streaming (game-loop) API: begin_stream() re-queues the tile
 * stream after any scene edit (stale tiles keep showing the previous frame —
 * no black flash); step_stream() renders until the budget is spent and
 * returns 1 while work is pending; stream_present() tonemaps the partial
 * buffer; rt_blit() upscales it into a window frame. */
NE_EXPORT void ne_stream_begin(NEEngine* e, int target_passes);
NE_EXPORT int  ne_stream_step(NEEngine* e, int budget_us);
NE_EXPORT int  ne_stream_active(NEEngine* e);
