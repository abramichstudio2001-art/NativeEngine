...
public:
    ...
    void begin_stream(int spp, uint32_t target_passes) {
        ...
        int tiles_x = (width + TILE - 1) / TILE;
        int tiles_y = (height + TILE - 1) / TILE;
        prev_passes = passes;                       // stale tiles present as last frame
        prev_passes.resize((size_t)tiles_x * (size_t)tiles_y, 0u);
    ...
    // Reset one tile's accumulator before its first fresh sample, so a scene
    // edit never flashes black: untouched tiles keep presenting the previous
    // frame while refreshed tiles blend the new state in progressively.
    void clear_tile_accum(uint32_t tile_id) {
        int tiles_x = (width + TILE - 1) / TILE;
        ...
        for (int y = y0; y < y1; ++y)
            for (int x = x0; x < x1; ++x) {
                ...
                accum[gi] = accum[gi + 1] = accum[gi + 2] = 0.0f;
            }
    }
    ...
    bool stream_step(int budget_us) {
        ...
        for (uint32_t n = 0; n < strm.total; ++n) {
            ...
            if (passes[id] == 0) clear_tile_accum(id);
    ...
    void present_stream() {
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                ...
                uint32_t div = p ? p : (tid < (uint32_t)prev_passes.size() ? prev_passes[tid] : 0);
                if (div == 0) {   // never rendered at all -> dark gray placeholder
                    rgba[j] = rgba[j + 1] = rgba[j + 2] = 24; rgba[j + 3] = 255; continue;
                }
                float inv = exposure * inv_spp / (float)div;
...
private:
    ...
    std::vector<uint32_t> prev_passes;
